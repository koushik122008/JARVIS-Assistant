...
def _gemini_client():
    """Return a Gemini client using the shared factory."""
    return new_gemini_client(model="gemini-2.5-flash")
...
def _process_text_doc(path: Path, file_type: str, action: str,
    ...
    # Free-form request (e.g. "translate to Turkish") → keep the original
    # action as the instruction so the LLM knows what the user asked.
    # "custom" is appended AFTER resolution so it captures the real instruction.
    if action not in prompt_map and action != "custom":
        if not instruction:
            instruction = action
        action = "custom"
    ...
    try:
        if file_type == "csv":
            try:
                ...
                # pandas >= 3.0 removed the `errors` kwarg from read_csv
