...
from utils import get_api_key, WIN_HIDE as _WIN_HIDE
...
def _xrandr_brightness(delta: float) -> None:
    """Adjust Linux brightness via xrandr using argument lists only (no shell)."""
    try:
        ...
        if display:
            ...
            match = re.search(r"Brightness:\s*([\d.]+)", verbose.stdout)
            current = float(match.group(1)) if match else 1.0
            new_val = max(0.1, min(1.0, current + delta))
            subprocess.run(
                ["xrandr", "--output", display, "--brightness", f"{new_val:.2f}"],
                capture_output=True,
            )
    except Exception as e:
        print(f"[Settings] Brightness (xrandr) failed: {e}")
...
def _detect_action(description: str, _client=None) -> dict:
...
def computer_settings(
    ...
    if not raw_action and description:
        from google import genai as _genai
        _client = _genai.Client(api_key=get_api_key())
        detected   = _detect_action(description, _client)
