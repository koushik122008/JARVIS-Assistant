...
def _get_model(model_name: str):
    """Create a Gemini client wrapper for the given model name."""
    return new_gemini_client(model_name)
...
def _open_vscode(project_dir: Path) -> bool:
    ...
    for cmd in vscode_candidates:
        try:
            # Resolve bare names via PATH first (on Windows 'code' is a .cmd shim,
            # which CreateProcess cannot execute directly).
            resolved = shutil.which(cmd) or cmd
            if resolved.lower().endswith((".cmd", ".bat")):
                # .cmd/.bat cannot be executed directly by CreateProcess —
                # run through cmd.exe with a safe argument list (no shell interpolation)
                subprocess.Popen(
                    ["cmd", "/c", resolved, str(project_dir)],
