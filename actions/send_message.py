...
def _whatsapp_deep_link(receiver: str, message: str) -> str:
    """Official wa.me deep link — opens a pre-filled chat for a phone number."""
    from urllib.parse import quote_plus
    number = re.sub(r"[\s\-().]", "", receiver)
...
def _send_whatsapp(receiver: str, message: str) -> str:
    # Phone numbers → official wa.me deep link (pre-filled, press Enter to send)
    if _PHONE_RE.match(receiver.strip()):
        url = _whatsapp_deep_link(receiver, message)
        if _open_browser_url(url):
            return (
                f"Prepared WhatsApp chat for {receiver.strip()} — "
                "message pre-filled, just press Enter."
            )
...
def _resolve_platform(platform_str: str):
    """Map a platform string to its send handler.

    Uses word-boundary matching so short aliases ('ig', 'wp') never match
    inside unrelated words (e.g. 'signal' must not become Instagram).
    """
    key = platform_str.lower().strip()
    for keywords, handler in _PLATFORM_MAP:
        if any(re.search(rf"\b{re.escape(k)}\b", key) for k in keywords):
            return handler
...
def send_message(
    ...
    receiver     = params.get("receiver", "").strip().lstrip("@")
    message_text = params.get("message_text", "").strip()
    platform     = params.get("platform", "whatsapp").strip()
    dry_run      = str(params.get("dry_run", "")).strip().lower() \
        in ("1", "true", "yes", "y")
    ...
    if not platform:
        return (
            "Please specify a platform (whatsapp, telegram, instagram, "
            "signal, discord, messenger, or any app name)."
        )

    preview = message_text[:50] + ("…" if len(message_text) > 50 else "")

    # Dry run: validate resolution without touching the desktop
    if dry_run:
        _resolve_platform(platform)
        return f"[DRY RUN] Would send \"{preview}\" to {receiver} via {platform}."
