...
class _BrowserSession:
    ...
    async def next_tab(self) -> str:
        return await self._cycle_tab(1)

    async def prev_tab(self) -> str:
        return await self._cycle_tab(-1)

    async def _cycle_tab(self, delta: int) -> str:
        ctx = self._context
        if not ctx:
            return "No browser context open."
        pages = ctx.pages
        if len(pages) < 2:
            return "Only one tab open."
        try:
            idx = pages.index(self._page) if self._page else 0
        except ValueError:
            idx = 0
        self._page = pages[(idx + delta) % len(pages)]
        try:
            await self._page.bring_to_front()
        except Exception:
            pass
        return f"Switched to tab: {self._page.url}"
...
def browser_control(
    ...
    if action in ("go_to", "search", "new_tab"):
        ...
        elif action == "new_tab" and not params.get("url", "").strip():
            # New tab without a URL → send the OS shortcut so a fresh tab opens
            # in the user's real, focused browser (no automation window needed).
            keys = ("command", "t") if _OS == "Darwin" else ("ctrl", "t")
            if _native_hotkey(*keys):
                result = f"new_tab (native): sent {'+'.join(keys).upper()}."
                _log(player, result)
                return result
            nav_url = ""   # hotkey unavailable → fall through to native launch
        else:
    ...
    # ── Tab / navigation shortcuts — on the user's REAL browser ──────────────
    # When the user opened sites natively (their own browser + profile) there is
    # NO automation session. Pure-shortcut actions (close_tab, next_tab, prev_tab,
    # back, forward, reload) must then send the OS shortcut instead of spawning a
    # whole automation window that acts on tabs the user can't even see.
    if action in _NATIVE_SHORTCUTS and not _registry.has(browser):
        keys = _NATIVE_SHORTCUTS[action]
        if _native_hotkey(*keys):
            result = f"{action} (native): sent {'+'.join(keys).upper()}."
        else:
            result = (
                f"Could not {action} — no browser automation session is active "
                f"and the keyboard shortcut could not be sent."
            )
        _log(player, result)
        return result
...
# Pure-shortcut actions sent to the user's REAL browser as OS key presses
# when no Playwright automation session is active.
_NATIVE_SHORTCUTS: dict[str, tuple] = {
    "close_tab": ("command", "w") if _OS == "Darwin" else ("ctrl", "w"),
    "next_tab":  ("command", "shift", "bracketright") if _OS == "Darwin" else ("ctrl", "tab"),
    "prev_tab":  ("command", "shift", "bracketleft") if _OS == "Darwin" else ("ctrl", "shift", "tab"),
    "back":      ("command", "left") if _OS == "Darwin" else ("alt", "left"),
    "forward":   ("command", "right") if _OS == "Darwin" else ("alt", "right"),
    "reload":    ("command", "r") if _OS == "Darwin" else ("f5",),
}


def _native_hotkey(*keys: str) -> bool:
    """Send a keyboard shortcut to the OS — works on the user's REAL browser.

    Fallback for pure-shortcut actions (close_tab, back, forward, reload) when
    the browser was opened natively (real profile, no Playwright session).
    Mirrors computer_settings.close_tab() / go_back() / refresh_page().
    """
    try:
        import pyautogui
        ...
        print(f"[Browser] ⚠️  Native hotkey {'+'.join(keys)} failed: {e}")
        return False
