...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        # .bat cannot be executed directly by CreateProcess — run via cmd.exe
        # with a safe argument list (no shell interpolation)
        try:
            r = subprocess.run(
                ["cmd", "/c", bat_path], capture_output=True, timeout=8
            )
            if r.returncode == 0:
...
# ── plugin stats (habits / expenses / workouts) ────────────────────────────────

def _read_plugin_json(name: str) -> dict:
    """Safely load a plugin store from memory/. Returns {} on any error."""
    try:
        return json.loads((BASE_DIR / "memory" / name).read_text(encoding="utf-8"))
    except Exception:
        return {}


def _streak_days(days: list) -> int:
    """Consecutive-day streak ending today (or yesterday — same rule as the
    habit plugin, so a skipped today doesn't kill the streak early)."""
    if not days:
        return 0
    logged = set(days)
    cursor = date.today()
...
def _stats_habits() -> dict:
    data   = _read_plugin_json("habits.json")
    habits = data.get("habits", {}) or {}
    today  = date.today().isoformat()
    week0  = (date.today() - timedelta(days=7)).isoformat()
    rows   = []
    for name, info in habits.items():
        log = info.get("log", []) or []
        rows.append({
            "name":       name,
            "streak":     _streak_days(log),
            "total":      len(log),
            "done_today": today in log,
            "week":       sum(1 for d in log if d >= week0),
            "created":    info.get("created", ""),
        })
    rows.sort(key=lambda r: (not r["done_today"], -r["streak"]))
    return {
        "habits":     rows,
        "total":      len(rows),
...
def _stats_expenses() -> dict:
    data    = _read_plugin_json("expenses.json")
    entries = data.get("entries", []) or []
    month   = date.today().strftime("%Y-%m")
    week0   = (date.today() - timedelta(days=7)).isoformat()
    month_e = [e for e in entries if (e.get("date") or "").startswith(month)]
    total   = sum(float(e.get("amount") or 0) for e in month_e)
    by_cat  = {}
    for e in month_e:
        c = e.get("cat", "other")
        by_cat[c] = by_cat.get(c, 0) + float(e.get("amount") or 0)
    cats = sorted(
        ({"cat": c, "amount": round(a, 2),
          "pct": round(a / total * 100, 1) if total else 0}
         for c, a in by_cat.items()),
        key=lambda x: -x["amount"],
    )
    budget = (data.get("budget", {}) or {}).get(month)
    recent = sorted(
        (
            {
                "date":   e.get("date", ""),
                "ts":     e.get("ts", ""),
                "desc":   (e.get("desc") or "").strip() or e.get("cat", "other"),
                "cat":    e.get("cat", "other"),
                "amount": round(float(e.get("amount") or 0), 2),
            }
            for e in entries
        ),
        key=lambda r: (r["date"], r["ts"]),
        reverse=True,
    )[:10]
    return {
        "month":       month,
        ...
        "week_total":  round(sum(float(e.get("amount") or 0)
                                 for e in entries if (e.get("date") or "") >= week0), 2),
        "by_category": cats,
        "recent":      recent,
    }


def _stats_workouts() -> dict:
    data    = _read_plugin_json("workout_log.json")
    entries = data.get("entries", []) or []
    week0   = (date.today() - timedelta(days=7)).isoformat()
    week    = [e for e in entries if (e.get("date") or "") >= week0]
    by_ex   = {}
    for e in entries:
        ex   = (e.get("exercise") or "Exercise").strip() or "Exercise"
        sets = int(e.get("sets") or 0)
        reps = int(e.get("reps") or 0)
        wt   = float(e.get("weight") or 0)
        d    = by_ex.setdefault(ex, {
            "exercise": ex, "sets": 0, "reps_total": 0, "volume": 0.0,
            "pr_weight": 0.0, "pr_date": "", "last": "",
        })
        d["sets"]       += sets
        d["reps_total"] += sets * reps
        d["volume"]     += sets * reps * wt
        if wt > d["pr_weight"]:
            d["pr_weight"] = wt
            d["pr_date"]   = e.get("date", "")
        if e.get("date", "") > d["last"]:
            d["last"] = e.get("date", "")
    ex_list = sorted(by_ex.values(), key=lambda x: -x["volume"])
    for x in ex_list:
        x["volume"] = round(x["volume"])
    recent = sorted(
        (
            {
                "date":     e.get("date", ""),
    ...
    return {
        "week_days":     len({e.get("date") for e in week}),
        "week_sets":     sum(int(e.get("sets") or 0) for e in week),
        "week_volume":   round(sum((int(e.get("sets") or 0))
                                   * (int(e.get("reps") or 0))
                                   * (float(e.get("weight") or 0)) for e in week)),
        "week_entries":  len(week),
...
<p>Connecting to JARVIS…</p>
        ...
        @app.get("/api/gallery")
        async def gallery(req: Request):
            if not _auth(req):
                return JSONResponse({"error": "Unauthorized"}, status_code=401)
            
            gallery_dir = Path.home() / "Pictures" / "Jarvis"
            images = []
                    ...
                    for f in sorted(
                        ...
                        if f.suffix.lower() in ('.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp'):
                            images.append({
                                ...
                                "url": f"/gallery/{f.name}?token={req.headers.get('authorization', '').removeprefix('Bearer ').strip()}"
                            })
                except Exception:
                    pass
        ...
        async def gallery_image(filename: str, token: str = ""):
            tok = token.strip()
            if not tok or tok not in self._tokens:
                return JSONResponse({"error": "Unauthorized"}, status_code=401)
        ...
        @app.get("/api/stats")
        async def plugin_stats(req: Request):
            """Aggregated plugin stats for the dashboard (habits/expenses/workouts)."""
            if not _auth(req):
                return JSONResponse({"error": "Unauthorized"}, status_code=401)
            return JSONResponse({
                "habits":   _stats_habits(),
                "expenses": _stats_expenses(),
                "workouts": _stats_workouts(),
            })
